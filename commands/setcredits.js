...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('setcredits')
		.setDescription('Sets credits for a user')
		...
		.addIntegerOption(option => 
			...
			.setDescription('Specify the amount of guild credits to set')
			.setRequired(true))
	...
	async execute(interaction) {
		...
		member = Credits.update({ credits: interaction.options.getInteger('credits') }, { where: {uuid: uuid} });
		const embed = new EmbedBuilder()
		.setColor("Red")
		.setTitle("Success!")
		.addFields({name: "Set " + username + "'s credits.", value: username + " now has " + interaction.options.getInteger('credits') + " credits."})
		...
		.setFooter({text: "Bot made by IdkDom#5897", iconURL: "https://cdn.discordapp.com/avatars/1101610763057102929/4dfb55a51e44eea78b6e6259e7b98578.webp?size=160"});
